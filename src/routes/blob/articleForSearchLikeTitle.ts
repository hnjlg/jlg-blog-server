	...
	app.post(
		...
		(req: Request, res: Response) => {
			...
			mysqlUTils.query<[string, number, number], []>(
				`SELECT blob_article.id, blob_article.title, blob_article.content, blob_article.reading_quantity, blob_article.add_time, article_status.status_name, article_status.status_value, 
				GROUP_CONCAT(article_tags.tag_name) AS tags FROM blob_article 
				...
				WHERE blob_article.valid = 1 AND blob_article.title LIKE ?
				GROUP BY blob_article.id 
				LIMIT ? OFFSET ?;`,
